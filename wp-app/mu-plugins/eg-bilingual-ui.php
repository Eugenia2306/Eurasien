...
add_filter(
	...
	function ( $content ) {
		...
		foreach ( $pairs as $needle => $bi ) {
			...
			// Do not inject spans into <option> / attribute values.
			if ( preg_match( '/<(option|optgroup)\\b[^>]*>[^<]*' . preg_quote( $needle, '/' ) . '/i', $content ) ) {
				continue;
			}
			$content = str_replace( $needle, eg_ui_bi( $bi[0], $bi[1] ), $content );
		}
...
(function () {
  var pairs = [
    ...
    { sel: 'button[name="add-to-cart"], .single_add_to_cart_button', de: 'In den Warenkorb', en: 'Add to cart' },
